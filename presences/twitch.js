...
client.on("ready", async() => {
    console.logger.info(`Đã kết nối ${client.user.tag}!`)
    ...
    console.logger.info('Twitch RPC Đã được kích hoạt!');
    console.logger.info('Twitch: ' + config.twitch.state);
    console.logger.info(`Status: ${!config.status ? 'mặc định' : config.status}`)
})